<!-- ================= HEADER ================= -->
<div align="center">

# Hi, I'm opsmon
**DevOps Engineer**

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=2600&pause=700&center=true&vCenter=true&width=820&lines=DevOps+Engineer+on+the+Payment+Service;Building+reliable+platforms+and+internal+developer+tools;MLOps+%7C+Observability+%7C+Automation+%7C+Developer+Experience;Kubernetes+Documentation+Contributor" alt="Typing intro" />

</div>

---

## 🧭 About Me

- 🏢 **X5 TECH** - DevOps Engineer on the Payment Service
- 🛠 **Focus:** MLOps, observability, automation, developer experience
- 🔧 **Core stack:** Linux, Docker, Kubernetes, Prometheus, Grafana, Argo CD, GitLab, PostgreSQL, Kafka, Python, n8n
- 🎓 **Student internships:** Acting as a speaker and helping students with growth, development, and onboarding inside the company
- 📋 **Passion:** Standardizing and documenting processes
- 💰 **FinOps:** Experience with cloud cost optimization and financial management practices
- ⚡ **Fun fact:** *I love documentation*

## 🎨 Brand System

- [opsmon Brandbook](BRANDBOOK.md) - visual identity, color tokens, typography, UI rules, and voice guidelines for the site.

---

## 🔗 Connect with Me

<div align="center">
<a href="https://t.me/opsmon_sh">
  <img src="https://img.shields.io/badge/Telegram-26A5E4?style=for-the-badge&logo=telegram&logoColor=white" alt="Telegram" />
</a>
<a href="mailto:opsmon.sh@yandex.com">
  <img src="https://img.shields.io/badge/Email-FF0000?style=for-the-badge&logo=yandex&logoColor=white" alt="Email" />
</a>
<a href="https://www.linkedin.com/in/opsmon/">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>
<a href="https://habr.com/ru/users/opsmon/">
  <img src="https://img.shields.io/badge/Habr-65A3BE?style=for-the-badge&logo=habr&logoColor=white" alt="Habr" />
</a>
</div>

---

## 🏆 Contributions

- ✍️ **Habr Author** - Published an article in the X5 Tech blog: [Debugging Remote Work: A DevOps Approach to Health](https://habr.com/ru/companies/X5Tech/articles/1045434/)
- 🧭 **Kubernetes Documentation Translator** - Translating official Kubernetes documentation into Russian
- ☁️ **Yandex Cloud Documentation Contributor** - Contributing to official Yandex Cloud documentation

---

⭐ *We live in a time when we need to keep growing endlessly.*
